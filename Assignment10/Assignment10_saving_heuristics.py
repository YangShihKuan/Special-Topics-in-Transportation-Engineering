    ...
    while route_capacity < capacity:
        ...
        for j in start_sort_list:
            if i!=j  and route_time+distant[i-1][j-1] < due_date[j]:
                saving_time = depot_distant[i]+depot_distant[j]-distant[i-1][j-1]
                saving_list.append(saving_time)
            else:
                saving_list.append(0)
    
        #print saving_list
        
        if sum(saving_list) == 0 or len(saving_list) == 0:
            break        
        next_index = saving_list.index(max(saving_list))
